...
class ScraperEngine:
    ...
    def _process_link(self, link, proxy, mirror, timeout, retries, budget, pattern, stats):
        ...
        if (kind == "html" or kind == "other") and is_github_blob:
            ...
            self.log(f"GitHub blob 链接 {link.split('/')[-1]} 返回页面，强制转 raw 重抓")
            if raw_url and raw_url != link:
                content2, err2 = self._fetch(raw_url, proxy, None, timeout, retries, budget)
                if not err2 and content2 and self._sniff(content2) == "m3u":
                    content = content2
                    kind = "m3u"
                else:
