...
class EpgService:
    ...
    def load_epg_batch(self, urls_with_proxy):
        ...
        threading.Thread(target=self._load_worker_batch,
                         args=(urls_with_proxy,), daemon=True).start()
        return {"loading": True}
    ...
    def _load_worker_batch(self, urls_with_proxy):
        ...
        for i, (url, proxy) in enumerate(urls_with_proxy):
            if not url or not url.strip():
                continue
            url = url.strip()
            self.log_callback(f"开始加载 EPG({i+1}/{len(urls_with_proxy)}): {url}")
            content, err = download_url(url, proxy if proxy else None)
            if err:
                self.log_callback(f"EPG 下载失败({i+1}): {err}")
                continue
            try:
                    ...
                    if name in epg_map:
                        ...
                        for p in info.get('programs', []):
                            if p['start'] not in existing_starts:
                                epg_map[name]['programs'].append(p)
                                existing_starts.add(p['start'])
                    else:
                        epg_map[name] = info
                total_channels += len(parsed)
                self.log_callback(f"  EPG({i+1}) 解析完成: {len(parsed)} 个频道")
            except Exception as e:
                self.log_callback(f"  EPG({i+1}) 解析异常: {e}")
        self.epg_data = epg_map
        self.epg_count = len(epg_map)
        self.epg_loaded = True
        self.save_cache()
        self.log_callback(f"EPG 多源加载完成，共 {len(epg_map)} 个频道（来自 {len(urls_with_proxy)} 个源，合并去重后）")
        self.epg_loading = False
    ...
    def _load_worker(self, url, proxy=""):
        ...
        try:
            ...
            if not self.epg_data:
                ...
                for name, info in parsed.items():
                    if name in self.epg_data:
                        existing_starts = {p['start'] for p in self.epg_data[name]['programs']}
                        for p in info.get('programs', []):
                            if p['start'] not in existing_starts:
                                self.epg_data[name]['programs'].append(p)
                    else:
                        self.epg_data[name] = info
            self.epg_count = len(self.epg_data)
            self.epg_loaded = True
            self.save_cache()
            self.log_callback(f"EPG 加载完成，共 {self.epg_count} 个频道")
        except Exception as e:
