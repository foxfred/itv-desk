...
def find_players():
    ...
    return {
        "vlc": os.environ.get("VLC_PATH") or shutil.which("vlc.exe") or _find_common(_VLC_PATHS),
        "pot": os.environ.get("POTPLAYER_PATH") or shutil.which("potplayermini.exe") or shutil.which("potplayer.exe") or _find_common(_POT_PATHS),
        "mpv": os.environ.get("MPV_PATH") or shutil.which("mpv.exe") or _find_common(_MPV_PATHS),
    }


def _find_common(paths):
    for p in paths:
        if os.path.isfile(p):
            return p
    return None
