...
- **v3.2.0** (2026-09-16): 功能大版本。新增 **录制 / 时移**（播放时可直接录制当前频道，也可开启时移缓冲随时暂停、回拖）；节目单里已结束的节目支持一键 **回看**（自动按源的 catchup 规则拼回看地址）；频道列表新增 **收藏** 与「只看收藏」筛选；新增 **频道墙**（`/wall`）多画面 —— 一屏铺满各频道的画面缩略图与延迟数值，点击直接播放，支持分组筛选 / 只看收藏 / 隐藏死源 / 列数可调 / 批量抓帧；新增 **家长控制**（设置 PIN 后可锁定指定分组，播放前需校验）；新增 **AI 智能分组** —— 设置页「AI 智能」填写模型 API 地址、Key（支持一键从 `/v1/models` 获取模型列表）、超时 / 温度 / 最大 token 等参数，频道列表可让模型分析频道名给出分组建议并一键应用；新增 **HDHomeRun 仿真** —— 开启后 Plex / Jellyfin / Emby / Kodi 可将本机当作调谐器添加并观看直播（含 lineup / device.xml / auto/v<号> 端点、调谐器数量限制、转码开关、SSDP 可选）
